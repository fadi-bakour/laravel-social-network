...
<form action="/news/ {{ $news->id }}" method="post" enctype="multipart/form-data">
    ...
    <div class="mr-5 mt-3">
        ...
        <textarea class="form-control" name="title" id="title" cols="25" rows="2"> {{ $news->title }}</textarea>
        @error('title')
    ...
    <div class="mr-5 mt-3">
        ...
        <textarea class="form-control" name="body" id="body" cols="30" rows="8" > {{ $news->body }}</textarea>
        @error('body')
            <p class="text-danger " >{{ $message }}</p>
        @enderror
    </div>
